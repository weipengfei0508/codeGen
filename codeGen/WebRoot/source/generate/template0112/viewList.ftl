...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><#list tbmodel as column><th>${column.column_comment}</th></#list><shiro:hasPermission name="${permissionPrefix}:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${r"${page.list}"}" var="${className}">
			<tr>
				<#list tbmodel as column><th>${"${"+className+"."+column.column_name+"}"}</th></#list>
