...
import com.whty.platform.generate.codegen.SqlServerProvider;
...
public class SqlGenerate {
	...
	private static Logger logger = LoggerFactory.getLogger(SqlGenerate.class);
	...
	public static void main(String[] args) throws Exception {
		...
		SqlServerProvider provider = new SqlServerProvider(jdbcConfig);
		
		
		String classAuthor = "zrt"; // 类作者
		// 生成代码所在的包
		String packageName = "com.ztxy.dwps";
		// 单张表生成代码
		String moduleName = "users"; // 模块名，例：sys
		String tablename = "dt_user_account"; // 表名
		String functionName = "会员信息表"; // 功能名，例：用户
		...
		GenCode(datebase, provider, packageName, moduleName,subModuleName, className, classAuthor, functionName,tablename);
	

	}

	public static void GenCode(String datebase, SqlServerProvider provider,
		...
		// 从数据库中得到该表的所有字段
		List<TableModel> tblist = provider.getTableModelList(tablename,
				datebase);
		...
		writeFile(content, filePath + "Mapper.java");
		logger.info(filePath);
		...
		content = FreeMarkers.renderTemplate(template, tbmodel);
		writeFile(content, filePath + "Set.Java");
		logger.info(filePath);
