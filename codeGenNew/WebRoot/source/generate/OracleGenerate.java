...
import com.whty.platform.generate.codegen.OracleProvider;
 ...
 * @version 2013-03-15
 */
public class OracleGenerate {

	private static Logger logger = LoggerFactory
			.getLogger(OracleGenerate.class);
	...
	public static void main(String[] args) throws Exception {
		...
		OracleProvider dbProvider = new OracleProvider(jdbcConfig);
		String classAuthor = "whty"; // 类作者，

		// 单张表生成代码
		String moduleName = "pay"; // 模块名，例：sys
		String subModuleName = ""; // 子模块名（可选）
		String className = "PayRefund"; // 类名，例：user
		String functionName = "退款信息表"; // 功能名，例：用户
		String tablename = "PAY_REFUND"; // 表名
	...
	public static void GenCode(String datebase, OracleProvider dbProvider,
		...
		String classPath = new DefaultResourceLoader().getResource("")
				.getFile().getPath();
		String templatePath = classPath.replace(separator + "build" + separator
				+ "classes", separator + "src" + separator + "com" + separator
				+ "whty" + separator + "platform" + separator + "modules");
		String javaPath = classPath.replace(
				separator + "build" + separator + "classes",
				separator
						+ "src"
						+ separator
						+ (StringUtils.lowerCase(packageName)).replace(".",
								separator));
		String viewPath = classPath.replace(separator + "build" + separator
				+ "classes", separator + "WebRoot" + separator + "WEB-INF"
				+ separator + "views");

		// 代码模板配置
		Configuration cfg = new Configuration();
		cfg.setDirectoryForTemplateLoading(new File(templatePath.replace(
				"modules", "generate" + separator + "template2")));
		// 从数据库中得到该表的所有字段
		...
		System.out.println(tablename + "代码生成成功！");

		// 生成 Entity
//		template = cfg.getTemplate("getset.ftl");
//		content = FreeMarkers.renderTemplate(template, tbmodel);
//		filePath = javaPath + separator + tbmodel.get("moduleName") + separator
//				+ "temp" + separator + StringUtils.lowerCase(subModuleName)
//				+ separator + tbmodel.get("ClassName") + ".java";
//		writeFile(content, filePath);
		// logger.info(filePath);
		System.out.println(filePath);
	}
